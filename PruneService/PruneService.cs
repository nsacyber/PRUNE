...
namespace PruneService
{
    public partial class PruneService : ServiceBase
    {
		...
		private readonly System.Timers.Timer _logTimer = new System.Timers.Timer();
		...
		protected override void OnStart(string[] args)
        {
			...
			try
            {
                //Check for old, logged files as the service starts
                RemoveOldLogs();
            }
            catch (Exception e)
            {
				Prune.HandleError(true, 1, "Error removing old logged files\n" + e.Message);
            }
			...
			//Create and start the timer to monitor logged files to remove
            try
            {
                _logTimer.Elapsed += (sender, e) => { RemoveOldLogs(); };
                _logTimer.Start();
            }
            catch (Exception e)
            {
				Prune.HandleError(true, 1, "Error starting the monitor logged files timer\n" + e.Message);
            }
        }
                    ...
                    foreach (int pid in fileLists.Last().Values.Distinct())
                    {
                        ...
                        lock (_PruneInstances)
                        {
                            ...
                            else
                            {
                                ...
                                string[] filePathSplit = processFiles[0].Split('\\');
                                string fileName = filePathSplit[filePathSplit.Length - 1];
                                string whitelistEntry = fileName.Split('-')[0].Split('_')[0];
                    ...
                    if (dict != null)
                    {
                        foreach (int pid in dict.Values.Distinct())
                        {
                            ...
                            lock (_PruneInstances)
                            {
                                ...
                                {
                                    //Extract the file name from the full path 
                                    string[] filePathSplit = filesList[0].Split('\\');
                                    string fileName = filePathSplit[filePathSplit.Length - 1];
                                    string whitelistEntry = fileName.Split('-')[0].Split('_')[0];
